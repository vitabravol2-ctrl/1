...
async function refresh(){
  ...
  el('topStatus').textContent=`${status.mode} | AI OFF/SIM | Binance OFF | Server ${status.server}`;
  el('runtime').textContent=JSON.stringify(runtime,null,2);
  const m = market[runtime.selectedPair]||{};
  el('market').innerHTML=`<h3>${runtime.selectedPair} (DEMO)</h3><p>Price: ${m.price}</p><p>Spread: ${m.spreadPct}%</p><p>Volatility: ${m.volatility}</p><p>Trend: ${m.trend}</p><p>Volume: ${m.volume}</p>`;
